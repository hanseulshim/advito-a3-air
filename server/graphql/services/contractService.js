...
class AuthenticationService {
   ...
   * @param {string} sessionToken Session token of user to authorize.
   */
  async authenticate(sessionToken) {
    // Gets session info from DB
    const sessionRow = await this.knex('advito_user_session')
      .select(
    ...
    const roleRows = await this.knex('advito_user_role_link')
    ...
    await this.knex('advito_user_session')
